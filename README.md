...
I have run through over 100,000 tests with n=k and the conjecture has held.
But I understand that is not a proof.

## ****Questions****

1.  For n = k = n<sub>0</sub> ,is there a method to
    solve for all n, k where f(n,k) = V\_{
    n<sub>0</sub> }, i.e. S\_{ n<sub>0
    </sub>}?
2.  Is there a method to determinethe number of solutions
    to f(n,k) = V\_{ n<sub>0</sub> }? i.e.
    O(n<sub>0</sub>).
3.  Can Conjecture 2 be proven? Was conjecture 2 already
...
sheets were generated by first running the Rust language program, main.rs, then
using the main.rs program in factors project to calculate the prime factors in the square
roots of the results, and finally running a python program to read the resultant
postgresSQL tables created by those to extract the spreadsheet data.

The main.rs program in this project iterates over integer pairs n,k where n=k=n<sub>0</sub>. 
After generating f(n,k) a search is done for all additional n <> k pairs where 
f(n,k) = f(n<sub>0</sub>,n<sub>0</sub>) = the same V\_(n<sub>0</sub> value.
The database key fields contain not the f(n<sub>0</sub>) values but the √f(n<sub>0</sub>). 
As shown in the proof of conjecture 1 above, the count of f(n,k) = f(n<sub>0</sub>,n<sub>0</sub>) 
values will be odd.
